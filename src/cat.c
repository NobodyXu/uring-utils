 ...
 */
static int spice1(struct io_uring *ring, int in_fd)
{
    for (struct io_uring_sqe *sqe; (sqe = io_uring_get_sqe(ring)); ) {
        io_uring_prep_splice(sqe, in_fd, (loff_t) -1, out_fd, (loff_t) -1, 1024, 0);
 ...
 */
int spice2(struct io_uring *ring, int in_fd)
{
    int pipefd[2];
    if (pipe(pipefd) < 0) {
        warn("pipe failed");
    ...
    for (struct io_uring_sqe *sqe; (sqe = io_uring_get_sqe(ring)); ) {
        if (!in_fd_consumed) {
            io_uring_prep_splice(sqe, in_fd, (loff_t) -1, pipefd[1], (loff_t) -1, 1024, 0);
        ...
        io_uring_prep_splice(sqe, pipefd[0], (loff_t) -1, out_fd, (loff_t) -1, 1024, 0);
        sqe->flags |= IOSQE_ASYNC;
        sqe->user_data = 1;
...
int main(int argc, char* argv[])
{
    ...
    if (is_in_fd_pipe || is_out_fd_pipe)
        exit_status = spice1(&ring, in_fd);
    else
        exit_status = spice2(&ring, out_fd);
